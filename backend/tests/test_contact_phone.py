...
def raise_ticket(api, item, **extra):
    return api.post(
        reverse("ticket-list"),
        {"service_item": item.pk, "description": "no power", **extra},
        format="json",
    )


def test_contact_phone_defaults_to_the_requesters_profile_number(
    api, requester, nrb_section, item, priorities
):
    api.force_authenticate(requester)
    response = raise_ticket(api, item)
...
def test_formatting_is_stripped_so_the_number_is_dialable(
    ...
    api.force_authenticate(requester)
    response = raise_ticket(api, item, contact_phone=given)
    assert response.status_code == 201
...
def test_a_user_with_no_number_can_still_raise_a_ticket(
    api, nrb, nrb_section, item, priorities
):
...
def test_blank_is_accepted_when_sent_explicitly(
    ...
    api.force_authenticate(requester)
    response = raise_ticket(api, item, contact_phone="")
    assert response.status_code == 201
    assert (
